...
var gameScene, healthBar, innerBar, outerBar, gameOverScene, message, id, dungeon, door, treasure, explorer, blobs, blob, state, collision, explorerHit, stars;
...
function setup() {
    ...
    message = new Text(
        "The End!",
        { fontFamily: "Futura", fontSize: "48px", fill: "white" }
    );
...
function play() {
    ...
    blobs.forEach(function (blob) {
        ...
        if (blobHitsWall) { if (blobHitsWall === "top" || blobHitsWall === "bottom") { blob.vy *= -1; } };
        if (b.hitTestRectangle(explorer, blob)) { explorerHit = true };
    });
    if (explorerHit) { explorer.alpha = 0.5; healthBar.outer.width -= 1; } else { explorer.alpha = 1; };
    if (b.hitTestRectangle(explorer, treasure)) { treasure.x = explorer.x + 13; treasure.y = explorer.y + 8; };
    if (b.hitTestRectangle(treasure, door)) { state = end; message.text = "You won!"; stars.play(); };
